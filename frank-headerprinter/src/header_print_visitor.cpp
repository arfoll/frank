#include "header_print_visitor.h"
...
{
  std::cout << nchars(indent_character_, indentation_) << "include \"" << node.package_name() << "\"" << std::endl;
  visit_children(node, *this);
}
...
{
  std::cout << nchars(indent_character_, indentation_) << "// WOOO" << std::endl;
  visit_children(node, *this);
}
